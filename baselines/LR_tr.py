...
base_path = "D:\my\mypaper\CausalBike\EXP\\capital\\LR\\in\\"
...
def load_train(demand_tag):
    ...
    for f in train_set:
        ...
        x= np.reshape(df['treat'].values,[-1,1])
...
for i in range(14):
    test_x,test_y,st_index =next(generator)
    if np.size(test_y)>0:
