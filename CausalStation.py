...
class CausalStation(tf.keras.Model):
    def __init__(self,cluster_dim,rnn_dim,hid_dim):
        super(CausalStation,self).__init__()
        ...
        # station embedding
        self.his_gru = layers.GRU(rnn_dim,kernel_initializer=tf.initializers.glorot_normal(seed=1))
        self.loc_Dense = layers.Dense(2,kernel_initializer=tf.initializers.glorot_normal(seed=1))
        self.cl_Dense = layers.Dense(rnn_dim,kernel_initializer=tf.initializers.glorot_normal(seed=1))
        ...
        self.mTc_Dense = layers.Dense(rnn_dim,kernel_initializer=tf.initializers.glorot_normal(seed=1))

        # Confonduers to treatments
        self.treat_Dense = layers.Dense(1,kernel_initializer=tf.initializers.glorot_normal(seed=1))

        # effect parameters
        self.ew_Dense = layers.Dense(1,kernel_initializer=tf.initializers.glorot_normal(seed=1))
        self.conf_Dense = layers.Dense(1,kernel_initializer=tf.initializers.glorot_normal(seed=1))
        self.no_treatd_Dense = layers.Dense(1,kernel_initializer=tf.initializers.glorot_normal(seed=1))
